...
All treatments except `FIL100` and `UST6` are significantly more efficacious than 
`PBO` at inducing clinical remission. We can now estimate the probabilities of each treatment being at each possible rank and the SUCRAs (Surface Under the Cumulative RAnking curve):
