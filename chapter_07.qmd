...
required_packages <- c("table1", "cobalt", "WeightIt", "optmatch",
                       ...
                       "interactionR", "MatchIt", "interactions")
...
W.out <- weightit(ps.formula.with.int, 
                  ...
                  method = "ps", 
                  estimand = "ATT")
d.w <- survey::svydesign(~1, weights = W.out$weights, data = Obs.Data)
fit2w <- survey::svyglm(hypertension ~ smoking*income, design = d.w,
                family = binomial("logit"))
...
d.w1 <- survey::svydesign(~1, weights = trimmed.weight.1.percent1, 
                          data = subset(Obs.Data, income == 1))
fit2unadj1 <- survey::svyglm(hypertension ~ smoking, design = d.w1,
                             family = binomial("logit"))
...
d.w0 <- survey::svydesign(~1, weights = trimmed.weight.1.percent0, 
                          data = subset(Obs.Data, income == 0))
fit2unadj0 <- survey::svyglm(hypertension ~ smoking, design = d.w0,
                             family = binomial("logit"))
...
fit2unadj.sb <- survey::svyglm(hypertension ~ smoking*income, design = d.w.sb,
                               family = binomial("logit"))
