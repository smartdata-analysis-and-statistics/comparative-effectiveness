...
true_noHTE_ATE <- getest(data = data_noHTE, estimandv = "ATE", Tform = 1, CC = TRUE, approachv = "Reference")
true_noHTE_ATT <- getest(data = data_noHTE, estimandv = "ATT", Tform = 1, CC = TRUE, approachv = "Reference")

ref_noHTE <- rbind(true_noHTE_ATE,true_noHTE_ATT)
```
...
fit_cc <- getest(data = mdata_noHTE, 
                 ...
                 approachv = "Complete Case Analysis")
...
fit_MInd <- getest(data = mdata_noHTE, 
                   ...
                   approachv = "Missing Indicator")

alldata <- rbind(ref_noHTE, fit_cc, fit_MInd) %>% filter(estimand == "ATE")

alldata$approachv = factor(alldata$approachv,
                           labels = c("Reference", "Complete Case Analysis", "Missing Indicator"),
                           levels = c("Reference", "Complete Case Analysis", "Missing Indicator"))

ggplot(alldata,aes(x= approachv, y= estimate, col = approachv)) +
  geom_point(shape = 1,
             size  = 1) +
