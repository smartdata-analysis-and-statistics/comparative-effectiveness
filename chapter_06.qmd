...
exp(coefci(weighted.fit2, 
       level = 0.95, # 95% confidence interval
       vcov. = vcovHC)["treatmentDMT1",])
```

Using this approach, the ATT estimate was `r round(exp(coef(weighted.fit2))["treatmentDMT1"],2)` (95% CI: `r paste(round(exp(coefci(weighted.fit2, level = 0.95, vcov. = vcovHC)["treatmentDMT1",]),2), collapse = "; " )`).
...
set.seed(1854)
arr.boot <- boot(dat, statistic = ps.reg.bootstrap, R=1000) # 1000 bootstrap replicates
median(arr.boot$t) # median annualized relapse rate across 1000 bootstrap replicates
quantile(arr.boot$t[,1], c(0.025, 0.975)) # Take 2.5th and 97.5th percentiles to be 95% CI
```
