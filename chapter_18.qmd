...
There are two R scripts for this chapter:

* `chapter16.R` is used as the main script that contains very similar code to this tutorial;
* `chapter16_functions.R` contains supporting functions that the main script uses.

### Setting up and define constants
```{r constant, eval = T, echo = T}

source("resources/chapter18_functions.R")
...
The data were simulated using function `simcountdata()` with 2000 sample size and randomization seed 63 so we all have the exact same dataset. 
```{r readdata, eval = T, echo = T}
set.seed(base.seed)
...
```{r summary, echo = F, eval = T, results = "asis"}
...
saveRDS(modlist2, "./Outputs/intermediate/modlist2.RData")
saveRDS(modlist3, "./Outputs/intermediate/modlist3.RData")
saveRDS(modpm, "./Outputs/intermediate/modpm.RData")
saveRDS(modpm.s, "./Outputs/intermediate/modpm.s.RData")
...
cvmodall0 <- cvvalue(data = df, xvar = covars,
                     method = "all0", n.fold = n.fold, n.cv = n.cv, seed = base.seed)

cvmodall1 <- cvvalue(data = df, xvar = covars,
                     method = "all1", n.fold = n.fold, n.cv = n.cv, seed = base.seed)

cvmoddwols <- cvvalue(data = df, xvar = covars,
                      method = "dWOLS", n.fold = n.fold, n.cv = n.cv, seed = base.seed)

cvmodpois <- cvvalue(data = df, xvar = covars,
                     method = "poisson", n.fold = n.fold, n.cv = n.cv, seed = base.seed)

cvmodlist2 <- cvvalue(data = df, xvar = covars,
                      method = "listDTR2", n.fold = n.fold, n.cv = n.cv, seed = base.seed) # very slow

cvmodcontrastreg <- cvvalue(data = df, xvar = covars,
                            method = "contrastReg", n.fold = n.fold, n.cv = n.cv, seed = base.seed) # very slow

saveRDS(modcv, "./Outputs/intermediate/modcv.RData")
saveRDS(cvmodall0, "./Outputs/intermediate/cvmodall0.RData")
saveRDS(cvmodall1, "./Outputs/intermediate/cvmodall1.RData")
saveRDS(cvmoddwols, "./Outputs/intermediate/cvmoddwols.RData")
saveRDS(cvmodpois, "./Outputs/intermediate/cvmodpois.RData")
saveRDS(cvmodlist2, "./Outputs/intermediate/cvmodlist2.RData")
saveRDS(cvmodcontrastreg, "./Outputs/intermediate/cvmodcontrastreg.RData")
``` 

### Clean up the results
Let's load the previously saved model results to save time. They are also available online for you to download.
```{r load, echo = T, eval = T}
modlist2 <- readRDS("./Outputs/intermediate/modlist2.RData")
modlist3 <- readRDS("./Outputs/intermediate/modlist3.RData")
modpm <- readRDS("./Outputs/intermediate/modpm.RData")
modpm.s <- readRDS("./Outputs/intermediate/modpm.s.RData")
modcv <- readRDS("./Outputs/intermediate/modcv.RData")
cvmoddwols <- readRDS("./Outputs/intermediate/cvmoddwols.RData")
cvmodpois <- readRDS("./Outputs/intermediate/cvmodpois.RData")
cvmodall1 <- readRDS("./Outputs/intermediate/cvmodall1.RData")
cvmodall0 <- readRDS("./Outputs/intermediate/cvmodall0.RData")
cvmodlist2 <- readRDS("./Outputs/intermediate/cvmodlist2.RData")
cvmodcontrastreg <- readRDS("./Outputs/intermediate/cvmodcontrastreg.RData")
```

Once we read in the prerun results, we need a preprocessing step to combine all estimated ITRs and value functions together.
```{r combine, echo = T, eval = T}
# Combine CV results
