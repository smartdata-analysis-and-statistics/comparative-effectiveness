...
results <- results %>% add_row(data.frame(study = "study 1",
                                          comparison = "B versus A",
                                          estimate = delta.1["d.2."],
                                          se = sqrt(diag(cov.1)["d.2."])))  
```
...
results <- results %>% add_row(data.frame(study = "study 2",
                                          comparison = "C versus A",
                                          estimate = delta.2["d.2."],
                                          se = sqrt(diag(cov.2)["d.2."])))   
```
...
results <- results %>% add_row(data.frame(study = "study 3",
                                          comparison = "B versus A",
                                          estimate = delta.3["d.2."],
                                          se = sqrt(diag(cov.3)["d.2."])))  
results <- results %>% add_row(data.frame(study = "study 3",
                                          comparison = "C versus A",
                                          estimate = delta.3["d.3."],
                                          se = sqrt(diag(cov.3)["d.3."])))  
```
...
We can now fit the second stage of the network meta-analysis:
```{r}
# second stage
second.stage<-
"model{
